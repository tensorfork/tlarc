...
#'(require (only-in math modular-expt))
...
#'(require (only-in file/md5 (md5 MD5)))
...
(or= srpasswords* (table))

(= srpwfile*   (libpath "arc/srpw"))

(defhook load-userinfo () name: srp
  (= srpasswords* (safe-load-table srpwfile*))
  t)

(defhook set-pw (pw (o user (get-user))) name: srp
  (k2-set-pw pw user))


(def srp-auth-req-from-A (A (o a))
  (obj A (hexint A)
       a (only.hexint a)))

(defsrp start-auth ((o a))
  (or= a (hexint:randhex 32))
  (def A (pow g a N))
  (srp-auth-req-from-A A a))

(defsrp server-start-auth (user (o b))
...
(def k2-auth (user req authfn)
  (or= req (srp-start-auth))
  (if (~isa!table req) (= req (srp-auth-req-from-A req)))
  (let res (k2-server-start-auth user nil)
    (fnidf (fn (proof)
             (withs (A req!A
                     B res!B
                     b res!b
                     ...
                     M (byteshex:k2-M user s A B S))
               (authfn user (is M proof))))
           (list 'k2-auth user))
    (inthex res!B)))

(def k2-auth-fn (subject)
  (aand (fnid-key (list 'k2-auth subject))
        (do1 (fns*:sym:fnkeys* it)
             (wipe (fns*:sym:fnkeys* it))
             (wipe (fnkeys* it)))))
...
;  (k2-check-pw auth: (srp-auth-req-from-A A) user: user))
